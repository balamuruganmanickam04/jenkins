...
public class AddCustomerPanel extends AbstractFunctionPanel {
	private final String[] header = new String[] { "S.N.", "Name", "Address", "PhoneNumber", "TECH ID" };
	...
	public static void main(String[] args) throws Exception {
		...
		EventQueue.invokeLater(() -> {
			try {
				...
				AddCustomerPanel panel = new AddCustomerPanel();
	...
	private void deleteSelectedBranchOffice() {
		try {
			DBUtils.deleteById(CustomerInfo.class, editingPrimaryId);
			changeStatus(Status.READ);
	...
	private CustomerInfo getModelFromForm() {
		CustomerInfo bo = new CustomerInfo();
	...
	private void setModelIntoForm(CustomerInfo bro) {
	...
	private void save(boolean isModified) {
		...
		if (v.validate()) {
			try {
				...
				CustomerInfo newBo = getModelFromForm();
				if (isModified) {
					CustomerInfo bo = (CustomerInfo) DBUtils.getById(CustomerInfo.class, editingPrimaryId);
	...
	private void readAndShowAll(boolean showSize0Error) {
		try {
			List<CustomerInfo> brsL = DBUtils.readAll(CustomerInfo.class);
			editingPrimaryId = -1;
	...
	private void showBranchOfficesInGrid(List<CustomerInfo> brsL) {
		dataModel.resetModel();
		int sn = 0;
		for (CustomerInfo bo : brsL) {
			dataModel.addRow(new Object[] { ++sn, bo.getName(), bo.getAddress(), bo.getPhoneNumber(), bo.getId() });
		}
	...
	private void populateSelectedRowInForm(int selectedId) {
		try {
			CustomerInfo bro = (CustomerInfo) DBUtils.getById(CustomerInfo.class, selectedId);
			if (bro != null) {
