...
public class AddVendorPanel extends AbstractFunctionPanel {
	...
	private void save(boolean isModified) {
		...
		if (v.validate()) {
			try {
				...
				if (isModified) {
					Vendor bo = (Vendor) DBUtils.getById(Vendor.class, editingPrimaryId);
	...
	private JPanel getUpperFormPanel() {
		if (formPanel == null) {
			...
			formPanel.setBorder(
					new TitledBorder(null, "Vendor Information", TitledBorder.LEADING, TitledBorder.TOP, null, null));
