 ...
 *         Copyright :
 *         <a href="http://ganeshtiwaridotcomdotnp.blogspot.com">Ganesh Tiwari
 *         </a>
 */
public class ResourceManager {
	...
	public static Map<String, String> readMap(String file, boolean isEncry) throws IOException {
		...
		while ((str = in.readLine()) != null) {
			...
			if (isEncry) {
				map.put(CryptographicUtil.decryptText(spl[0].trim(), a),
						CryptographicUtil.decryptText(spl[1].trim(), a));
			} else {
	...
	public static boolean saveMap(String file, Map<String, String> map, boolean isEncry) {
		...
		try {
			...
			for (Map.Entry<String, String> entry : map.entrySet()) {
				if (isEncry) {
					out.write(CryptographicUtil.encryptText(entry.getKey(), a) + ":"
							+ CryptographicUtil.encryptText(entry.getValue(), a) + "\n");
				} else {
	...
	public static String getConfigParam(String key) {
		if (configParamMap == null) {
			try {
				...
				JOptionPane.showMessageDialog(null, "Cannot Read Config File" + e.getMessage(), "Severe Error",
	...
	public static ImageIcon getAppIcon(String resourcePath) {
		return new ImageIcon(java.awt.Toolkit.getDefaultToolkit().getClass().getResource("/images/" + resourcePath));
	}
