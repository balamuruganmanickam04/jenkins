...
public class Validator {
	 ...
	 */
	public final synchronized void addTask(JComponent comp, String message, String regex, boolean isRequired,
			boolean showPopup) {
	...
	protected final boolean validationCriteria(Task task) {
		...
		if (jc instanceof JDateChooser) {
			...
			/*
			 * If this field can be empty and current input date is empty , then it is ok.
			 */
	...
	private boolean validate(Task task) {
		if (!validationCriteria(task)) {
			...
			if (parent instanceof Verifier)
				((Verifier) parent).validateFailed();
		...
		if (parent instanceof Verifier)
			((Verifier) parent).validatePassed();
